...
type SASLAuth struct {
	...
	SASLVersion   *int   `json:"sasl_version"`

	// GSSAPI config
	SASLGSSAPIServiceName        string `json:"sasl_gssapi_service_name"`
	SASLGSSAPIAuthType           string `json:"sasl_gssapi_auth_type"`
	SASLGSSAPIDisablePAFXFAST    bool   `json:"sasl_gssapi_disable_pafxfast"`
	SASLGSSAPIKerberosConfigPath string `json:"sasl_gssapi_kerberos_config_path"`
	SASLGSSAPIKeyTabPath         string `json:"sasl_gssapi_key_tab_path"`
	SASLGSSAPIRealm              string `json:"sasl_gssapi_realm"`

	// OAUTHBEARER config. experimental. undoubtedly this is not good enough.
	SASLAccessToken string `json:"sasl_access_token"`
}
